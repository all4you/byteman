...
public class StaticMethodAdvice {
    ...
    public static final InternalLogger logger = InternalLoggerFactory.getLogger(StaticMethodAdvice.class);

    /**
     * 在实例target的方法执行之前触发该方法调用
     *
     * @param interceptorName 被委托给的拦截器
     * @param clazz           被代理的实例对象
     * @param method          被代理的方法
     * @param args            被代理的方法参数
    ...
    public static <T> T beforeMethod(@InterceptorName String interceptorName, @Advice.Origin Class clazz, @Advice.Origin Method method, @Advice.AllArguments Object[] args, @Advice.Local("localValue") Object localValue) {
        T packet = null;
        try {
            StaticMethodInterceptor<T> interceptor = InterceptorFactory.newInterceptor(interceptorName);
            packet = interceptor.beforeMethod(clazz, method, args);
        } catch (Throwable e) {
            logger.warn("StaticMethodAdvice occurred error beforeMethod, cause=" + e.getMessage(), e);
        }
        return packet;
    }
    ...
    public static <T> void afterMethod(@InterceptorName String interceptorName, @Advice.Enter T packet, @Advice.Local("localValue") Object localValue, @Advice.Return(typing = Assigner.Typing.DYNAMIC) Object originResult, @Advice.Thrown Throwable throwable) {
        ...
        if (packet != null) {
            try {
                ...
                logger.warn("StaticMethodAdvice occurred error afterMethod, cause=" + e.getMessage(), e);
            }
        }
    }
